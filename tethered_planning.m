...
build_cost_map;
plot(Traj(:,1),Traj(:,2),'b.'); hold on;
plot(Traj(:,4),Traj(:,5),'b.');
...
while 1>0
    ...
    N=size(Traj,1)-2;
    options=optimoptions('fmincon','MaxFunctionEvaluations',1e5,'Display','iter','Algorithm','interior-point');
    X_opt=fmincon(@(X_opt)cost(X_opt,s,e,A,circles,X2_ini,R_in),X_ini,[],[],[],[],[],[],@(X_opt)mynonlcon(X_opt,X2_ini,circles,A4,B4),options);
    if (mod(step,2)==1)
...
function [c,ceq]=mynonlcon(Traj,Traj2,circles,A4,B4)
   N=size(Traj,1);
   c(1:N,1)=-Traj(:,1);
   c(N+1:2*N,1)=-Traj(:,2);
